...
- [x] **M7.0 规划（Plan）**：基于真实 Apple Silicon 0.9.11 崩溃、`--doctor`、发布包文件 mode 与未提交实现的独立审计完成规划。范围限定为 macOS 维护锁、Unix 权限复制、Node/npx 定位、进程组与私有工作目录、pkg 结果传播/用户环境/暂存清理、WKWebView 数据清理、macOS 14+ 产物目标、崩溃符号和真实产物门禁；Windows 现有安装行为作为回归基线，不在本里程碑重写。
- [x] **M7.1 人工批准（Approve）**：维护者已审阅修订后的 M7 范围、执行顺序、风险、验证证据与回退策略，并明确授权继续修正。
- [x] **M7.2 跨平台维护锁（Execute）**：保留 Windows 已验证行为；macOS 使用不依赖 `TMPDIR` 的按当前用户稳定租约目录和 `FileShare.None` 排他租约。首次启动先通过维护门，安装器持有维护租约且旧实例退出后，第三个客户端不能取得实例租约；覆盖安装、卸载和跨 `await` 持有。
- [x] **M7.3 Unix 负载权限（Execute）**：部署 `.app` 时复制普通文件和目录 Unix mode，并在 staging 与最终 `InstallRoot` 分别验证主程序及卸载命令的用户执行位；权限缺失测试会失败并回滚，不提交 `0644` 主程序。
- [x] **M7.4 macOS Node/npx 解析（Execute）**：统一解析器返回并验证绝对 `node`/`npx` 对及受控执行 PATH；安装前置检查、`--doctor`、Finder 启动和 LaunchAgent 登录启动均用同一环境执行 `--version` 与 DSH，覆盖 Homebrew 和一种用户级版本管理器，诊断记录来源类别、路径和版本，不记录完整环境。
- [x] **M7.5 spawn-time 进程所有权（Execute）**：macOS npx 通过 `posix_spawn` 在执行前建立独立 process group，并在同一启动边界应用受管 `launcher-cwd`；父进程只验证 `getpgid`，不再以 `setpgid(childPid, childPid)` 追赶；绑定失败只终止本次创建的 PID。
- [x] **M7.6 macOS pkg 事务闭环（Execute）**：postinstall 在有效控制台用户会话中以显式目标用户环境同步等待 Avalonia bootstrap，保存可定位诊断、传播真实退出码，并通过 trap 在成功、失败和中断时精确删除系统 bootstrap；失败不启动受管客户端。
- [x] **M7.7 macOS WebView 与卸载（Execute）**：WebKit 原生清理 helper 在 macOS 14+ 清理产品 identifier 存储；卸载事务在删除产品路径前同步完成平台清理。
- [x] **M7.8 包元数据与诊断（Execute）**：`.app` bundle 类型、`.icns` 和 WebKit helper 完整；主程序和 helper 的 Info.plist/Mach-O 最低版本均为 14.0，构建阶段验证 arm64、执行位、`@rpath` dylib、部署目标与 Info.plist，并将匹配 dSYM 保存到内部验证目录而不装入用户 pkg。
- [ ] **M7.9 自动验证（Verify）**：真实 `macos-14` arm64 Runner 执行包结构、AOT、维护锁、权限复制、Node 解析、私有工作目录、进程组所有权和 postinstall 结果传播门禁；工作流只使用版本控制的 ReleaseTests 与打包脚本，绝不引用 LocalTests。
- [x] **M7.10 Windows 回归（Verify）**：运行现有 `win-x64` 构建、安装事务、单实例、运行中替换、DSH 启停和卸载验证，确认共享接口调整未改变当前稳定的 Windows 行为。
...
1. **先修锁与运行环境**：macOS 主实例必须通过维护门；租约固定到不依赖 `TMPDIR` 的稳定、按当前用户命名的根目录。Node/npx 作为带受控 PATH 的可执行对验证，并在 spawn 边界设置私有工作目录和 process group。
