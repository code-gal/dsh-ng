...
- **V2.3 成功提交**：只有客户端文件就位、DSH 启动、Web UI 身份验证通过、自启动、系统卸载入口和 Windows 当前用户桌面/开始菜单快捷方式注册成功后，安装器才报告成功。
...
- **V2.10 运行中保留数据替换**：在运行中的 .NET 依赖版上启动 AOT 安装器，及在运行中的 AOT 版上启动 .NET 依赖安装器；确认安装器先完成 IPC 停止、WebView 销毁、DSH 停止和互斥体交接，再替换客户端文件。IPC 拒绝、旧实例未退出或超时均显示可复制的失败原因，不出现无响应窗口、不强杀无关进程且旧安装保持可启动。
- **V2.11 本地安装版本识别**：新版本、同版本、较旧版本和 AOT/.NET 依赖切换分别显示升级、修复、降级警告和构建形态变化；无元数据旧清单仍可选择保留数据安装；安装器不访问网络或下载客户端更新。
- **V2.12 AOT 旧安装检测响应性**：在已安装 .NET 依赖版且客户端仍运行时打开 AOT 安装器，旧安装清单和版本检测期间窗口持续响应重绘、移动与关闭；完成检测后出现正确的升级/修复/切换选项，读取失败显示可关闭错误，不需要结束进程。
- **V2.13 macOS 安装结果传播**：分别模拟 bootstrap 正常提交、启动崩溃、Node 缺失、无控制台用户和用户停止；macOS Installer 必须同步获得对应退出码，失败不得显示安装成功，诊断不得被重定向到 `/dev/null`。
- **V2.14 Unix 权限保持**：以包含 `0755` 主程序和卸载命令的 macOS payload 执行部署，staging 与最终 `.app` 均保持用户执行位；人为移除源权限或模拟 mode 设置失败时事务必须回滚且不注册登录项。
...
- **V3.8 macOS Node 解析一致性**：在终端、Finder 等价环境、pkg 当前用户会话和最小 PATH 的 LaunchAgent 环境分别运行同一解析器；Apple Silicon Homebrew 与至少一种用户级版本管理器的 node/npx 均解析为已验证绝对路径，`--doctor` 与实际启动结果一致。
...
- **V4.10 安装维护锁**：安装器在运行中客户端退出前不能部署或替换 `InstallRoot`；获得维护锁后，安装期间不会创建第二个受管 DSH 实例或被新的客户端启动竞态打断。
- **V4.11 macOS spawn-time 进程组**：高频重复启动短生命周期子进程，确认 npx 执行前已建立独立 process group，不出现 exec 后 `setpgid` 的 `EACCES` 竞态；优雅与强制停止只影响该组，旁路 Node 进程持续运行。
...
- **V6.5 Windows 快捷方式**：成功安装后当前用户桌面和开始菜单均存在“DSH Desktop”快捷方式，目标和图标来自受管安装目录；失败回滚和卸载后两处快捷方式均消失。
- **V6.6 macOS 登录环境**：成功安装后注销并重新登录，LaunchAgent 在不依赖交互式 shell PATH 的情况下启动客户端和 DSH，保持窗口隐藏且菜单栏可用；系统设置中禁用后覆盖安装不重新启用。
...
- **V7.8 macOS pkg 残留**：成功、失败和中断恢复路径结束后，`/Library/Application Support/DSH Desktop Installer` 不保留 bootstrap 或客户端副本；完整卸载后用户 `.app`、LaunchAgent、产品数据和 WKWebView 数据均不存在。仅允许 `pkgutil` 显示无对应 payload 的系统历史 receipt。
