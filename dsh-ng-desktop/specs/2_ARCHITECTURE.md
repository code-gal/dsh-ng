...
- **发行渠道**：仅通过 GitHub Releases 发布版本化安装包、校验值和签名状态；Windows `win-x64` 与 macOS `osx-arm64` 均发布醒目标记的未签名社区预览。
- **平台范围**：Windows `win-x64` 与 macOS `osx-arm64` 是唯一发布和验收目标；Windows ARM64、macOS Intel 与其他 RID 只保留源码兼容性。
- **发行记录**：`CHANGELOG.md` 是人工维护的版本与用户可见更新来源；`desktop-v<SemVer>` 标签必须与其中的版本条目一致，CI 只校验和消费该版本，不自动决定版本语义。
...
- Native AOT 的项目验证范围固定为真实 `win-x64` 与 `osx-arm64`；不维护 `win-arm64`、`osx-x64` 或其他 RID 的构建矩阵。
...
Windows `win-x64` 产出 AOT 和 .NET 依赖安装器；macOS `osx-arm64` 产出 Native AOT `pkg`。安装流程、状态机、日志和 DSH 调度逻辑保持跨平台可复用；未支持架构的平台适配代码仅作为源码兼容性实现，不产出、发布或验证项目发行物。
...
- 安装负载清单和 `InstallManifest` 保存 `ProductVersion` 与 `BuildFlavor`；版本字段采用可静态分析的 SemVer 比较器，未知或旧清单按兼容分支处理。安装窗口只根据本地元数据生成升级、修复、降级和构建形态切换文案，不执行网络版本检查。
...
- 保留数据的替换路径必须先调用安装维护协作者：`SingleInstanceCoordinator` 增加安装停止命令，运行中客户端收到后拒绝新操作、销毁 WebView、停止 `DshSupervisor` 并关闭生命周期；安装器等待同一实例互斥体释放后持有维护锁，锁覆盖部署、DSH 供应和注册阶段，直至安装会话退出。请求或等待超时进入可见失败并保留旧安装，不允许在旧实例仍持有文件时调用部署替换。
- 安装界面通过只读位置服务打开已存在的 `InstallRoot`，或在部署前打开最近的现有父目录；该操作不得创建目录或改变事务状态。
- `SetupWindow` 的初始旧安装检测和包分类在后台工作线程完成，只把最终状态投递回 Avalonia UI；禁止在 `Opened` 事件或其他 UI 回调中对 `InstallManifest.LoadAsync` 使用同步等待。安装事务本身同样不占用 UI 线程。
...
- `SingleInstanceCoordinator` 除激活和卸载命令外还承载安装维护停止命令。运行中客户端收到卸载或安装维护请求后，立刻阻止新操作，销毁 WebView、停止 Supervisor 并关闭生命周期；卸载助手或安装器只在收到确认且确认单实例互斥体已释放后，才开始删除或替换文件。无运行实例时，调用方取得同一互斥体后可直接继续。未确认或超时必须以失败退出，不删除或替换文件。卸载清理仍分两阶段：先保留 `state/install-manifest.json` 并删除其余受管路径和安装根目录，最后才删除状态目录。
...
- 每个 `osx-arm64` 版本生成一个 Native AOT、自包含的 `DSH-Desktop-Setup-v<SemVer>-osx-arm64-aot.pkg`。`pkg` 安装临时 bootstrap 到系统受管位置，其 postinstall 在当前登录用户会话中启动同一 Avalonia 安装事务，目标固定为 `~/Applications/DSH Desktop.app`。
...
- 任何新增依赖和实现都必须在合并前通过 `win-x64` 和 `osx-arm64` 的 Native AOT 发布；非 AOT 成功不得作为豁免理由。
