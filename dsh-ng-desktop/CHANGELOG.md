...
- 修复 macOS 崩溃转储分析步骤可能长时间无输出卡住发布流程的问题：`dotnet-dump analyze` 增加 300 秒硬超时，超时或失败不影响原始转储工件上传。

## [0.9.15] - 2026-08-17

### Fixed

- 修复 macOS Release 门禁崩溃无法定位堆栈的问题：CI 在测试宿主崩溃时自动使用 `dotnet-dump` 分析崩溃转储，输出线程与托管调用堆栈并作为工件上传，用于精确定位 `posix_spawn` 启动器首次真实 macOS 执行时的崩溃位置。
