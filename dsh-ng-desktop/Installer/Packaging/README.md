...
所有安装器和 SHA-256 文件输出到 `artifacts/installer/`，该目录已由 Git 忽略，只能作为 GitHub Release 附件上传。无证书构建的 Windows EXE 可作为“未签名社区预览”附件上传，但 Release 必须展示 SmartScreen 风险、SHA-256 校验步骤和“不导入根证书”的说明。macOS 不生成项目发行安装包；外部开发者如需验证，可自行构建并承担验证责任。
